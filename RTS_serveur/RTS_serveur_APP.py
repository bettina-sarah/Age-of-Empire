...
class Dbman():
    ...
    def ajouteractionaujoueur(self, nom, cadrejeu, action):
        ...
        self.curs.execute("Insert into actionsenattente (nom,cadrejeu,action) VALUES(?,?,?);", (nom, cadrejeu, action_str))
        self.conn.commit()
...
def boucler_sur_jeu():
    db = Dbman()
    data = request.get_json()  # 'force=True' to parse even if the content-type header is not set to application/json

    # Access the data
    nom = data.get("nom")
    cadrejeu = int(data.get("iteration_boucle_jeu"))
    actionsrequises = data.get("actions_requises", [])
    #print("GOT THIS", nom,cadrejeu,actionsrequises)
    # cadreactuel=db.getinfo("cadrecourant")[0]
    #nom = request.form["nom"]
    #cadrejeu = int(request.form["iteration_boucle_jeu"])
    #actionsrequises = request.form["actions_requises"]
    #print("SERVEUR ACTIONS REQUISES RECUES",actionsrequises)
    ####################################################################
    ...
    if actionsrequises != "None":
        info = db.getinfo("joueurs")
        for i in info:
            print("INFO POUR BD", i[0], sautdecadre, actionsrequises)
            db.ajouteractionaujoueur(i[0], sautdecadre, actionsrequises)
            print("IN DB", i[0], sautdecadre, actionsrequises)
    ...
    if (cadrejeu - _min[1]) > 10:
        ...
        for i in info:
            if i[0] == nom:
                maliste.append([i[1], json.loads(i[2])])
